...
//    m_painter->setPen(QPen(backColor, 1, Qt::SolidLine, Qt::FlatCap));
        ...
        m_compensation.size() == m_return.size())
    {
        ...
        double step = static_cast<double>(m_geometry.width() - m_geometry.left() - FieldV) / static_cast<double>(m_compensation.size());
        ...
        qDebug() << lineComp0 << lineComp100 << m_geometry.top() << "  " << lineCompY << minComp << propComp;
...
//        m_painter->drawRect(x, y1, m_geometry.width() - x, h);
...
//            //! Компенсация
//            m_painter->setPen(QPen(getColorTransient(i, m_parComp), 2, Qt::SolidLine, Qt::FlatCap));
...
//            //! Возврат
//            m_painter->setPen(QPen(getColorTransient(i, m_parRet), 2, Qt::SolidLine, Qt::FlatCap));
...
//                m_painter->drawLine(x1, 0, x1, lineRetY);
//                m_painter->drawText(x1 - 4, lineRetY + 15, QString::number(i / m_freq));
//            }
//        }
...
//        m_painter->drawLine(x, 0, x, lineCompY);
